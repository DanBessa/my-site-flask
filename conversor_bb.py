...
def selecionar_pdfs(pdf_path=None):
    """ Processa um arquivo PDF específico ou permite seleção via janela se nenhum for passado. """
    
    if not pdf_path:  # Se nenhum caminho for passado, abre um seletor de arquivos
        root = tk.Tk()
        root.withdraw()
    ...
    # Pergunta o código do banco ao usuário (pode ser fixo no Flask se necessário)
    codigo_banco = simpledialog.askstring("Código do Banco", "Digite o código do banco:")
    if not codigo_banco:
        return "Nenhum código do banco fornecido"
    ...
    if not codigo_aplicacao:
        return "Nenhum código de aplicação fornecido"
    ...
    return f"Processamento concluído para {pdf_path}"
